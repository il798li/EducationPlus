...
public class DiscordUtility {
    ...
    public static void deletable (SlashCommandInteractionEvent slashCommandInteractionEvent, Display[] displays, boolean deletable) {
        ...
        WebhookMessageCreateAction <Message> webhookMessageCreateAction = interactionHook.sendMessage (message);
        webhookMessageCreateAction.queue ();
    }

    public static void deletable (final SlashCommandInteractionEvent slashCommandInteractionEvent, Display display) {
        final Display[] displays = new Display[1];
        displays[0] = display;
        deletable (slashCommandInteractionEvent, displays, false);
    }

    public static void respond (final SlashCommandInteractionEvent slashCommandInteractionEvent, final EmbedBuilder embedBuilder) {
    ...
    public static MessageEmbed embed (final SlashCommandInteractionEvent slashCommandInteractionEvent, final Display[] displays) {
        ...
        if (!guildSetting.compact) {
            embedBuilder.setTitle ("Education+");
        }
    ...
    public static User getUser(final JDA jda, String username) {
        username = username.toLowerCase ();
        final List<User> users = jda.getUsers ();
        for (final User user : users) {
            final String thisUsername = user.getName ();
            final boolean usernameMatches = thisUsername.equals (username);
            if (usernameMatches) {
                return user;
            }
        }
